#!/usr/bin/env python3
"""
SENTINELA Plugin — Sensitive File Detector v2
...
class SensitiveFilePlugin(SentinelaPlugin):
    ...
    def run(self, target: str, context: dict) -> list:
        ...
        # .git/HEAD acessível E confirmado (status 200) → reconstrói o repo
        # e escaneia a história completa com Gitleaks. Só faz isso porque o
        # próprio servidor já está expondo o .git publicamente — não é
        # comprometimento adicional, é o dado que já está público.
        git_exposto = any(p == ".git/head" and s == 200 for p, s, _ in
                           [(p.lower().lstrip("/"), s, sz) for p, s, sz in all_paths])
        if git_exposto:
            findings += self._dump_e_escanear_git(target, context)

        return findings

    def _dump_e_escanear_git(self, target: str, context: dict) -> list:
        import shutil, subprocess, os, json as jsonlib

        if not shutil.which("git-dumper"):
            self.log(".git exposto, mas git-dumper não instalado — dump/escaneamento de "
                     "histórico pulado (achado de .git/HEAD já reportado acima)", "warn")
            return []

        config = context.get("config", {}) or {}
        base_urls = context.get("base_urls") or [f"https://{target}"]
        base = base_urls[0]
        outdir = os.path.join(config.get("output", "sentinela_results"),
                               f"git_dump_{target.replace(':', '_').replace('/', '_')}")

        try:
            subprocess.run(
                ["git-dumper", base.rstrip("/") + "/.git/", outdir],
                capture_output=True, text=True, timeout=config.get("tool_timeout", 300),
            )
        except Exception as e:
            self.log(f"Erro ao rodar git-dumper: {e}", "error")
            return []

        if not os.path.isdir(os.path.join(outdir, ".git")):
            self.log("git-dumper não conseguiu reconstruir o repositório", "warn")
            return []

        if not shutil.which("gitleaks"):
            f = self.finding(
                severity="high", title=f"Repositório .git reconstruído em {outdir} — Gitleaks não instalado",
                detail="O repositório foi baixado com sucesso mas o Gitleaks não está instalado pra "
                       "escanear a história por segredos. Instale gitleaks e rode manualmente contra "
                       f"`{outdir}`.",
                url=base, remediation="Remover o .git do diretório público servido imediatamente.",
                confidence=0.95, impact=8.0,
            )
            return [f] if f else []

        try:
            gitleaks_out = os.path.join(outdir, "..", "gitleaks_report.json")
            subprocess.run(
                ["gitleaks", "detect", "--source", outdir, "--no-git=false",
                 "--report-format", "json", "--report-path", gitleaks_out],
                capture_output=True, text=True, timeout=config.get("tool_timeout", 300),
            )
        except Exception as e:
            self.log(f"Erro ao rodar gitleaks: {e}", "error")
            return []

        achados = []
        if os.path.exists(gitleaks_out):
            try:
                with open(gitleaks_out) as fh:
                    segredos = jsonlib.load(fh)
                for s in segredos[:20]:
                    f = self.finding(
                        severity="critical",
                        title=f"Segredo no histórico do Git: {s.get('RuleID', '?')} "
                              f"(commit {s.get('Commit', '?')[:10]})",
                        detail=f"Segredo tipo `{s.get('RuleID')}` encontrado no arquivo "
                               f"`{s.get('File')}` no commit `{s.get('Commit')}` — mesmo removido "
                               f"em commits posteriores, permanece na história completa do repositório.",
                        url=base,
                        evidence=f"{s.get('File')}:{s.get('StartLine')} — commit {s.get('Commit', '?')[:10]}",
                        remediation="Revogar/rotacionar o segredo IMEDIATAMENTE (presuma comprometido). "
                                    "Remover o .git do servidor público. Reescrever a história do repo "
                                    "(BFG Repo-Cleaner) não resolve o vazamento já ocorrido — é tarde "
                                    "demais pra esconder, só pra evitar recorrência.",
                        confidence=0.9, impact=9.5, exploitability="pre-auth",
                        business_context="Segredo já foi baixado por qualquer um que tenha acessado o "
                                          ".git público — tratar como comprometido, não só 'exposto'.",
                    )
                    if f:
                        achados.append(f)
            except Exception:
