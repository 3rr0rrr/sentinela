#!/usr/bin/env python3
"""
SENTINELA - Headless Browser Module
...
class HeadlessBrowser:
        ...
        async with async_playwright() as pw:
            ...
            for url in urls[:5]:  # limit to 5 base URLs
                try:
                    ...
                    log(f"      [!] Falha ao analisar {url} no navegador — "
                        f"resultado incompleto, não é o mesmo que \"sem DOM XSS\": {e}", Colors.YELLOW)
    ...
    async def _take_screenshot(self, page, url: str, results: dict):
        ...
        try:
            ...
            screenshots_dir = Path(self.config.get("output", "sentinela_results")) / "screenshots"
