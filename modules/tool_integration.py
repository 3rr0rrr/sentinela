#!/usr/bin/env python3
"""
SENTINELA - Tool Integration Layer
...
TOOL_REGISTRY = {
    ...
    # Vetores de exploração avançados (v1.1) — recon/enum passivo extra
    "subfinder":     ["subfinder"],
    ...
    # CVE/vuln scanners extras (v1.2)
    "osv-scanner":   ["osv-scanner"],
    "cve-bin-tool":  ["cve-bin-tool"],
    "nettacker":     ["nettacker", "nettacker.py"],
    "zap":           ["zap.sh", "owasp-zap", "zaproxy"],
    "gvm-cli":       ["gvm-cli"],
    "wcvs":          ["wcvs"],
...
class MasscanRunner:
    ...
    def scan(self, target: str, ports: str = "0-65535", rate: int = 1000,
        ...
        if rc not in (0, 1) and not ports_found:
            err = stderr[:200] if stderr else "masscan failed — may need root: sudo sentinela ..."
...
class CveBinToolRunner:
    """cve-bin-tool (OpenSSF) — escaneia BINÁRIOS compilados por assinatura
    de versão de biblioteca vulnerável conhecida (openssl, libpng, libxml2
    etc). Chamável por qualquer módulo que baixe um arquivo binário durante
    o scan (ex: um binário exposto publicamente encontrado no crawl)."""

    def __init__(self, runner: ToolRunner):
        self.r = runner

    def escanear_arquivo(self, caminho_local: str, timeout: int = 120) -> list:
        if not self.r.available("cve-bin-tool"):
            return []
        binario = self.r.which("cve-bin-tool")
        with tempfile.NamedTemporaryFile(suffix=".json", delete=False) as f:
            out_file = f.name
        try:
            rc, stdout, stderr = self.r.run(
                [binario, "--format", "json", "--output", out_file, caminho_local],
                timeout=timeout,
            )
            with open(out_file) as fh:
                dados = json.load(fh)
            achados = []
            # cve-bin-tool --format json retorna lista de achados diretamente
            # (ou dict com "results" dependendo da versão) — parse best-effort
            lista = dados if isinstance(dados, list) else dados.get("results", [])
            for item in lista:
                achados.append({
                    "produto":    item.get("product", "?"),
                    "versao":     item.get("version", "?"),
                    "cve":        item.get("cve_number", item.get("cve", "?")),
                    "severidade": (item.get("severity") or "MEDIUM").upper(),
                })
            return achados
        except Exception:
            return []
